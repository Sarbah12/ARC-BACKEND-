...
// Where course-registration alerts go. Override with ADMIN_NOTIFY_EMAIL
// (comma-separated for several recipients).
const NOTIFY_TO = (process.env.ADMIN_NOTIFY_EMAIL || 'accraresourcecenter@gmail.com')
  .split(',').map(a => a.trim()).filter(Boolean);

// Tells the ARC team a learner just registered (or updated an existing
// registration, e.g. after paying). Never throws — a failed notification
// must not break the learner's registration.
export async function sendRegistrationNotification({
  firstName, lastName, email, phone, course, mode, status, paymentRef, note, updated = false,
}) {
  if (!process.env.RESEND_API_KEY) {
    console.warn('[sendRegistrationNotification] RESEND_API_KEY not set — skipped');
    return false;
  }
  const appUrl = process.env.APP_URL || 'https://www.arcaccra.org';
  ...
  const row = (label, value) => value
    ? `<tr><td style="padding:6px 12px 6px 0;color:#6b7280;font-size:13px;white-space:nowrap;">${label}</td>
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      replyTo: email,
      subject: updated
        ? `Registration updated: ${name} — ${course}`
        : `New course registration: ${name} — ${course}`,
      text:
        `${updated ? 'A registration was updated' : 'A new course registration was received'}.\n\n` +
        `Name: ${name}\nEmail: ${email}\nPhone: ${phone || '—'}\nCourse: ${course}\n` +
        `Mode: ${mode || '—'}\nStatus: ${status || 'pending'}\n` +
        `${paymentRef ? `Payment ref: ${paymentRef}\n` : ''}${note ? `Note: ${note}\n` : ''}` +
        `\nView all registrations: ${appUrl}/admin`,
      html: `
        <div style="font-family:system-ui,Segoe UI,Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;">
          <p style="margin:0 0 4px;font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#6B4FD8;">
            ${updated ? 'Registration updated' : 'New course registration'}
          </p>
          <h2 style="margin:0 0 20px;font-size:20px;color:#111827;">${esc(name)} — ${esc(course)}</h2>
          <table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;background:#f9fafb;border-radius:10px;padding:8px 16px;">
            ${row('Name', name)}
            ${row('Email', email)}
            ${row('Phone', phone)}
            ${row('Course', course)}
            ...
            ${row('Payment ref', paymentRef)}
            ${row('Note', note)}
          </table>
          <p style="margin:22px 0 0;">
            <a href="${appUrl}/admin" style="display:inline-block;background:#6B4FD8;color:#fff;font-weight:700;font-size:14px;padding:11px 22px;border-radius:8px;text-decoration:none;">Open the admin panel →</a>
          </p>
          <p style="margin:18px 0 0;color:#9ca3af;font-size:12px;">Reply to this email to contact ${esc(name)} directly.</p>
        </div>
      `,
    });
    if (error) { console.error('[sendRegistrationNotification]', error); return false; }
    return true;
  } catch (err) {
    console.error('[sendRegistrationNotification]', err.message || err);
    return false;
  }
}
